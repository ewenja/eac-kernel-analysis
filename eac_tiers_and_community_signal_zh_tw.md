# EAC 分級與社群訊號 — 怎麼讀「免費版 vs 完整版」這類討論

> **章節級免責聲明：** 本檔是**來源判讀**，不是操作指南。
> 文末「附錄」節錄了一段社群成員的規避建議，那是**引用第三方說法**（已標示作者與日期），
> **不是本資料夾提供的作法**，也未經任何驗證。要照著做之前，請先看完整篇對照表。
...
| 項目 | 內容 |
|---|---|
| 標題 | *EAC free version detection vectors*（[Question] 分類） |
| 版面 / 討論串 | UnKnoWnCheaTs — Anti-Cheat Research / **#772975** |
| 發文者 | `asinio`（n00bie，註冊於 2026-09） |
| 時間 | 2026-09-20 ~ 2026-09-24（18 篇回覆） |
| 網址 | <https://www.unknowncheats.me/forum/anti-cheat-research/772975-eac-free-version-detection-vectors.html> |
| 性質 | **提問串**，不是研究貼文。沒有任何原始資料、log、位址或工具輸出 |
...
1. 「EAC 依遊戲分級、強度不同」這件事，又多了一個社群端的第一手情境（雖然是提問者的自述）。
2. 它印證了本資料夾那篇 EOS 研究**用的方法不是孤例** —— 串裡直接推薦用核心模擬器（kernemul）自己跑分析，跟本篇第一篇用的 KEVLAR 是同一類工具鏈。
3. 它是一個**資訊品質的活教材**：串裡一篇講得頭頭是道的技術主張，被其他成員指出是低階 LLM 生成的猜測，而且與本 repo 手上的證據不符。這正好呼應我們在原文封存開頭標的「AI-written」警語。

**不能拿來用的：**

- 它**不是偵測向量清單**。整串沒有任何一項附上證據、位址、工具輸出或可重現步驟。
- 它**不能當事實引用**。串內成員互相矛盾，而且部分說法與我們手上第一份逐指令級研究的觀察**直接衝突**（見下表）。
...
> **以下是 `bigslim04` 於 2026-09-23 在該討論串的發言節錄**，逐字引用、僅刪去論壇自動遮蔽的字眼與重複語句。
...
- 相關章節：[uc_eos_driver_writeup_zh_tw.md](uc_eos_driver_writeup_zh_tw.md) · [vulnerabilities_and_gaps.md](vulnerabilities_and_gaps.md) · [eac_beginner_report_zh_tw.md](eac_beginner_report_zh_tw.md)
