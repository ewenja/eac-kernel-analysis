...
不過先提醒一件事：**不是每款 EAC 遊戲都一定把這些檢查開到一樣滿。** 近年的社群觀察普遍認為，熱門、對抗強、作弊多的遊戲，通常會把保護配得更重；一些小型遊戲則可能沒那麼兇。這不代表某項功能只存在於某款遊戲，但很適合用來理解「為什麼別人的研究結果跟你手上的樣本不同」。
...
### 近期值得注意的方向：不只看記憶體，也可能看輸出結果

近年的外部研究有提到，在某些 EAC 樣本中可以看到與 Win32k / GDI 相關的解析路徑，例如：
...
這些 API 本身的用途，跟畫面擷取或把畫面資料拉回 buffer 很接近。所以比較合理的推測是：

- EAC 可能不只想知道「有沒有 overlay 掛著」
- 也可能想知道「最後顯示出來的畫面長什麼樣」

但這裡先不要寫死成「EAC 一定會截圖」。目前比較穩的說法是：

> 這是一條很值得重視的能力線索，可能和 overlay 檢查、畫面輸出驗證或條件式截圖有關，但是否量產啟用、何時觸發、是否上傳，還需要更多動態證據。
