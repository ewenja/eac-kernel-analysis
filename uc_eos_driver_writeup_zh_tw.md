# 外部研究導讀：EOS kernel driver 的硬體身分蒐集、核心遙測與 CPU 探針

> **免責聲明：** 本檔僅供安全研究與教育用途。內容整理自一篇公開發表的第三方逆向研究貼文，
> 屬於**外部來源、未經本 repo 重現或驗證**。本 repo 不提供作弊程式、漏洞利用或繞過手法。

---

## 一句話講完

2026 年 9 月在 UnKnoWnCheaTs（Anti-Cheat Research 版面）出現一篇非常長的 EOS kernel driver 逆向研究，
作者 `lauralex` 把 `eos.sys`（Fortnite 版本）丟進自製的 Windows 模擬環境 **KEVLAR** 裡執行，
跑出 **8,538 萬筆已驗證事件**，然後逐步拆出這支 driver 在「硬體身分蒐集、開機信任狀態、PCI/ACPI 探測、
ETW 遙測、反虛擬化 CPU 探針」各條路徑上實際做了什麼。

這篇的價值不在於它「公布了什麼神奇 bypass」，而在於**它是一份逐指令級的行為地圖**：
哪一個 RVA 讀了什麼、把資料編碼進哪個結構、用什麼演算法加密、掛在哪一條 list 上、什麼時候釋放。
對照本 repo 現有的靜態分析章節，它剛好補上「**同一條路徑在執行時長什麼樣子**」的那一塊。

- 原文全文（逐字封存）：[uc_eos_driver_writeup_original_en.md](uc_eos_driver_writeup_original_en.md)
- 原始討論串：<https://www.unknowncheats.me/forum/anti-cheat-research/772181-inside-eac-eos-driver-hardware-identity-collection-kernel-telemetry-cpu-probes.html>
...
| 作者 | `lauralex`（UnKnoWnCheaTs 會員） |
| 版面 / 討論串 | Anti-Cheat Research / #772181 |
| 發表時間 | 主文 2026-09-15；更新 2026-09-17；補充章節 2026-09-22（GMT） |
| 討論串規模 | 4 頁、72 篇回覆（本 repo 整理時，最後活動為 2026-09-24） |
...
本 repo 原本的分析主線是**靜態**逆向（IDA Pro、radare2、section/import/函式層級），
另外有一條 `2026-03-11` 樣本的靜態再驗證線。這一篇外部研究補的是**不同角度**：
...
這一節很重要。這篇的數字如果不先理解它的產生方式，很容易被誤讀成「真實主機上量到的值」。

### KEVLAR 模擬環境

作者不是在真機上跑 `eos.sys`，而是在 KEVLAR 這個模擬的 Windows 環境裡執行：
...
| 證據種類 | 意思 |
|---|---|
| Observed execution | 在某個已命名的 run 裡，確實有某條指令／呼叫／存取發生 |
| Modeled response | 回應是 KEVLAR 提供的；模型的限制會標在影響結論的地方 |
| Original-code control | 原始位元組在保留或宣告的狀態下被執行過；**「有一條分支存在」不等於「那條分支被執行過」** |

閱讀時建議照這個分類去看每一條結論，不要把它們壓成同一種確定性。
...
以下每一條都標了它是「觀察到的執行」還是「原始碼控制／重建」等級。**所有數字都是模擬環境產物。**

> 節號約定：這裡的 1～13 是**本導讀自己**的分類；文中若寫「原文第 N 節」，指的是原作者文章的節號
...
- 想找「它有沒有動 win32k」「它有沒有做 stack walk」這類問題的線索時，看上面的第 10 節（本導讀編號）
