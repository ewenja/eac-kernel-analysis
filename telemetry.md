...
但這邊有一個觀念很重要：**不是所有 IOCTL 或所有來回封包，都一定是同一條遙測通道。** 近年的外部分析有提到，有些固定長度封包更像 challenge-response 或 user-mode 驗證，有些才像真正搬資料的通道。換句話說，EAC 的「通訊」很可能是多層分工，不是單一水管做到底。
...
這表示 EAC 很重視的不只是「有沒有蒐集到資料」，還有「這份資料送出去前有沒有被動過」。
...
這裡也要補一句保守說法：184 bytes 這個數字在本樣本很重要，但**不應該直接拿去當所有 EAC build 的固定真理**。不同版本、不同遊戲、不同 runtime 展開狀態，封包格式很可能都會調整。
