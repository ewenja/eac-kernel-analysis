...
另外要先講一個很現實的前提：**這類加密參數和混淆常數不一定跨 build 通用。** 近年的 runtime 研究有提到，不同樣本在載入後可能出現不同的 resolver 常數、不同的虛擬化區段大小，甚至不同的展開結果。所以這份文件裡提到的位址、常數和流程，最好理解成「這個樣本看到的是這樣」，而不是「EAC 永遠都長這樣」。
